...
let TRACK={linksFormed:0,linkStrikesFired:0,linkBonusDmg:0,packsOpened:0,pawnSells:0,caEffects:0,
  ...
  // v0.7.1 Hangover system:
  hangoverHpDebuffApplied:0,hangoverCommits:0,hangoverWasted:0,hangoverStashHaircut:0,
  // v0.7.2 trip system:
  dmtBought:0,dmtUsed:0};
let CARD_PLAYS={};
...
function rollTrip(type){
  // v0.7.2: bunk removed, bad rate 5%→3%, pools expanded to 8 effects each.
  // DMT (premium tier, no bad pool). Returns symbolic constant (matches sim's
  // existing _tripBuff string convention with underscores).
  const roll=Math.random()
  if(type==='dmt'){
    TRACK.goodTrips++
    return['HYPERSPACE','OVERMIND','GODHEAD','REBIRTH','THIRD_EYE','SACRED_CHORD','TIMELINE_COLLAPSE','BLACK_SUN'][rand(8)]
  }
  if(roll<0.03){TRACK.badTrips++;return type==='shrooms'?'BAD_SHROOMS':'BAD_ACID'}
  TRACK.goodTrips++
  const d8=rand(8)
  if(type==='shrooms')return['EGO_DEATH','TIME_DILATION','SYNESTHESIA','COSMIC_UNITY','BLOTTER_REVELATION','PSILOCYBIN_PORTAL','DOOM_CRYSTAL','GHOST_WEED'][d8]
  return['FRACTAL_VISION','DIMENSIONAL_RIFT','EGO_DISSOLUTION','ASTRAL_PROJECTION','DMT_BREAKTHROUGH','REALITY_GLITCH','CRYSTAL_SHRIEK','K_HOLE'][d8]
}
...
function simFight(gs,phaseHp,luciferPhase){
  ...
  // v0.7.2: trip strike-mult initial values applied if trip already active
  const _initialMult=gs._tripBuff==='OVERMIND'?3.0:gs._tripBuff==='REALITY_GLITCH'?2.0:1.0
  gs._strikeMult=_initialMult;gs._cardsPlayedIds=[];gs._firedChains=new Set();gs._allCardsFree=gs._allCardsFree||false;gs._hellquakeFired=false
  ...
  // Members enter the next fight with -⌊hangover/33⌋ max HP each (capped at 3).
  // Restored on victory below (search "hangoverHpDebuff").
  const _hangHp=Math.min(3,Math.floor((gs.hangover||0)/33))
  if(_hangHp>0){
    gs.stage.forEach(m=>{
      if(!m||m.tooStoned||m.keyword==='FALLEN')return
      if(m.hangoverHpDebuff)return // failsafe: don't double-apply
      m.maxHp=Math.max(1,m.maxHp-_hangHp)
      m.hp=Math.min(m.hp,m.maxHp)
      m.hangoverHpDebuff=_hangHp
    })
    TRACK.hangoverHpDebuffApplied=(TRACK.hangoverHpDebuffApplied||0)+1
  }
  // ── PEAK CORRUPTION TRACKER (v0.7.1) ──
  // Reset live peak on fight start; commit to gs.hangover on victory.
  gs._peakCorruption=gs.corruption
  // (Whispers at 25% removed in v0.7.1 — Possession at 100% removed, replaced by Hangover system)
  ...
  // v0.7.2 TRIP logic — pools expanded, DMT tier added, bunk removed
  ...
  // DMT: prioritize over acid for the hardest fights — premium drug for premium moments.
  if(gs.heldDMT&&(fightIdx===26||gs._wthFight||(isBoss&&circleNum>=7))){
    const trip=rollTrip('dmt');gs.heldDMT=false;TRACK.dmtUsed=(TRACK.dmtUsed||0)+1
    gs._tripBuff=trip
    if(trip==='GODHEAD'){gs.stage.filter(m=>!m.tooStoned).forEach(m=>{m.atk+=10;m.tempAtkBonus=(m.tempAtkBonus||0)+10})}
    else if(trip==='REBIRTH'){gs.stage.forEach(m=>{if(m){if(m.tooStoned){m.tooStoned=false;m.hp=m.maxHp;m.atk=(m._origAtk!==undefined?m._origAtk:m.atk)+2}else{m.atk+=2}m.permAtkBonus=(m.permAtkBonus||0)+2}})}
    else if(trip==='THIRD_EYE'){gs.maxEmbers=Math.min(MAX_EMBERS_CAP,gs.maxEmbers+3);gs.embers=Math.min(MAX_EMBERS_CAP,gs.embers+3)}
    else if(trip==='TIMELINE_COLLAPSE'){maxStrikes=maxStrikes+2;gs._strikesLeft=maxStrikes}
    else if(trip==='HYPERSPACE'){gs._allCardsFree=true}
    // OVERMIND, SACRED_CHORD, BLACK_SUN — pure _tripBuff reads in combat loops
  } else if(gs.heldAcid&&(fightIdx>=21||fightIdx===26||gs._wthFight||(isBoss&&circleNum>=6)||(fightIsHard&&bandIsWeak))){
    ...
    else if(trip==='CRYSTAL_SHRIEK'){gs.stage.filter(m=>!m.tooStoned).forEach(m=>{m.atk+=5;m.tempAtkBonus=(m.tempAtkBonus||0)+5})}
    else if(trip==='DMT_BREAKTHROUGH'||trip==='K_HOLE'){gs._bossSkipStrikes=2}
    // FRACTAL_VISION, DIMENSIONAL_RIFT, ASTRAL_PROJECTION, REALITY_GLITCH — pure _tripBuff reads
    ...
    else if(trip==='BAD_SHROOMS'){gs.stage.filter(m=>!m.tooStoned).forEach(m=>{m.atk=Math.max(1,m.atk-1)})} // softened from -2 to -1
    else if(trip==='DOOM_CRYSTAL'){const _alive=gs.stage.filter(m=>!m.tooStoned);if(_alive.length){const _top=_alive.reduce((a,b)=>a.atk>b.atk?a:b);_top.tempAtkBonus=(_top.tempAtkBonus||0)+_top.atk;_top.atk*=2}}
    else if(trip==='BLOTTER_REVELATION'){gs._freeCardsLeft=3}
    // SYNESTHESIA, TIME_DILATION (handled below), PSILOCYBIN_PORTAL (instant draw not modeled — sim already draws to handSize each strike), GHOST_WEED — _tripBuff reads
  }

  // v0.7.2: TIME_DILATION + TIMELINE_COLLAPSE both grant bonus strikes
  ...
  for(let strike=0;strike<maxStrikes;strike++){
    ...
    // (75% Madness random-discard removed in v0.7.1 simplification)
    ...
    for(let att=0;att<15;att++){
      const playable=gs.hand.map((c,idx)=>({c,idx})).filter(({c})=>{
        let cost=c.embers-(gs._tripBuff==='SYNESTHESIA'?1:0);
        if(gs._tripBuff==='GHOST_WEED'&&c.type==='CORRUPT')cost=0
        if(gs._allCardsFree)cost=0;else if(!evilEyeUsed)cost=0;else if(gs._nextCardFree)cost=0;
        else if(gs._freeCardsLeft>0&&c.embers>0)cost=0;
      ...
      // v0.7.2: GHOST_WEED — CORRUPT cards cost 0 this fight
      if(gs._tripBuff==='GHOST_WEED'&&card.type==='CORRUPT')cost=0
      ...
      // v0.7.2: BLOTTER_REVELATION — counter-based free cards
      else if(gs._freeCardsLeft>0&&card.embers>0){cost=0;gs._freeCardsLeft--}
      ...
      // v0.7.2: BLACK_SUN — every CORRUPT card adds +50% strike multiplier
      if(card.type==='CORRUPT'&&gs._tripBuff==='BLACK_SUN'){
        gs._strikeMult=Math.min(10000,Math.round((gs._strikeMult*1.5)*100)/100)
      }
    ...
    if(gs._tripBuff==='SACRED_CHORD')strikeDmg*=3
    else if(gs._tripBuff==='DIMENSIONAL_RIFT'||gs._tripBuff==='FRACTAL_VISION')strikeDmg*=2;
    // Apply strike multiplier (0.03 per card played + 0.15 per combo)
    // Corruption power multiplier
    const corrMult=gs.corruption>=100?3.0:gs.corruption>=80?2.0:gs.corruption>=60?1.5:gs.corruption>=40?1.2:1.0 // v0.7.1: kept original ramp; cost moved to Hangover system
    ...
    // v0.7.2: trip strike-mult initial values — REALITY_GLITCH×2, OVERMIND×3
    const _tripStart=gs._tripBuff==='OVERMIND'?3.0:gs._tripBuff==='REALITY_GLITCH'?2.0:1.0
    gs._strikeMult=_tripStart;gs._cardsPlayedIds=[];gs._firedChains=new Set()
    if(gs.artifacts.some(a=>a.id==='a3'))gs._nextCardFree=true
    ...
    // BOSS ATTACKS
    let bossDmg=enemy.baseDmg+enemy._atkBuff+STAKE.dmgAdd; // v0.7.1: removed +3 Possession bonus at 100% corruption
    ...
    // v0.7.2: bossSkipStrikes counter — DMT_BREAKTHROUGH, K_HOLE, SACRED_CHORD effects
    if(gs._bossSkipStrikes>0){bossDmg=0;gs._bossSkipStrikes--}
    ...
    // Track peak corruption (commits to gs.hangover on victory below)
    if(gs.corruption>(gs._peakCorruption||0))gs._peakCorruption=gs.corruption
    if(gs.stage.every(m=>m.tooStoned))break;
  }

  const won=enemy._hp<=0,allDead=gs.stage.every(m=>m.tooStoned);
  if(won){
    // ── HANGOVER HP DEBUFF — restore on every fight victory (v0.7.1) ──
    gs.stage.forEach(m=>{
      if(m&&m.hangoverHpDebuff){
        m.maxHp=m.maxHp+m.hangoverHpDebuff
        m.hangoverHpDebuff=0
      }
    })
    gs.fightsSurvived++;
    ...
    if(!gs._wthFight){
      const ci=Math.min(circleNum-1,8),rewardBase=circleBaseMin[ci]+rand(circleBaseRange[ci]+1);
      const merch=gs.passives.some(p=>p.id==='p3')?2:0,corrB=gs.corruption>=69?3:0;
      // ── HANGOVER STASH HAIRCUT (v0.7.1) — calibrated via sim ──
      const _peakC=gs._peakCorruption||gs.corruption
      const _hairMult=_peakC>=100?0.85:1.0
      const totalBase=rewardBase+merch+corrB
      const total=Math.floor(totalBase*_hairMult)
      if(_hairMult<1.0)TRACK.hangoverStashHaircut=(TRACK.hangoverStashHaircut||0)+(totalBase-total)
      gs.stash=Math.min(MAX_STASH,gs.stash+total);gs.stashEarned+=total;
    ...
    // ── HANGOVER COMMIT (v0.7.1) ──
    // Boss kills clear hangover entirely, non-boss victories commit peak as next-fight cost.
    const _peakForCommit=gs._peakCorruption||gs.corruption
    if(isBoss){
      gs.hangover=0
    } else {
      gs.hangover=_peakForCommit
      if(_peakForCommit>=50)TRACK.hangoverCommits=(TRACK.hangoverCommits||0)+1
      if(_peakForCommit>=100)TRACK.hangoverWasted=(TRACK.hangoverWasted||0)+1
    }
...
// v0.7.1: Hangover shop tax (carried-over peak corruption from previous fight)
function simShop(gs){
  const _hO=gs.hangover||0
  const hungerMult=_hO>=100?1.60:_hO>=75?1.40:_hO>=50?1.20:1.00
  ...
  // v0.7.2: DMT — boss shops only, 25 stash, AI buys when affording
  if(isBossShop&&!gs.heldDMT&&gs.stash>=25&&gs.stash>=35){gs.stash-=25;gs.heldDMT=true;TRACK.dmtBought=(TRACK.dmtBought||0)+1}
...
function newGame(){
  ...
  return{stage:pickStartingPair(),deck:buildDeck(),discard:[],hand:[],stash:3,embers:_sE,maxEmbers:_sE,corruption:_sC,hangover:0,_peakCorruption:_sC,fightIndex:0,bossKills:0,artifacts:[],passives:[],fightsSurvived:0,totalDamage:0,highestStrike:0,stashEarned:0,tooStonedCount:0,won:false,mentorLinks:[],lastCircle:1,stashStolen:0,
  heldShrooms:false,heldAcid:false,heldDMT:false,_bossSkipStrikes:0,_freeCardsLeft:0,_tripBuff:null,stolenAtkPool:0,circleArtBought:false,circlePassBought:false,
...
// ── RUN SIMULATION ──
console.log(`\n⛧ VESTIBULE SIM v20.1-trips [${STAKE.name}] [${DECK_ID.toUpperCase()}] — ${NUM_GAMES.toLocaleString()} games\n`);
const t0=Date.now();
...
console.log(`\n🥴 HANGOVER SYSTEM (v0.7.1):`)
console.log(`  Hangover commits (50%+ peak carried fwd): ${TRACK.hangoverCommits.toLocaleString()} (${(TRACK.hangoverCommits/NUM_GAMES).toFixed(2)}/game)`)
console.log(`  WASTED (100%) commits: ${TRACK.hangoverWasted.toLocaleString()} (${(TRACK.hangoverWasted/NUM_GAMES).toFixed(2)}/game)`)
console.log(`  HP debuff applications: ${TRACK.hangoverHpDebuffApplied.toLocaleString()} (${(TRACK.hangoverHpDebuffApplied/NUM_GAMES).toFixed(2)}/game)`)
console.log(`  Stash lost to haircut: ${TRACK.hangoverStashHaircut.toLocaleString()} 🌿 (${(TRACK.hangoverStashHaircut/NUM_GAMES).toFixed(1)}/game)`)
console.log(`\n🎲 RANDOM EVENTS:`)
...
console.log(`\n🍄🧪💠 DEALER STATS (v0.7.2):`);
console.log(`  Shrooms bought: ${TRACK.shroomsBought.toLocaleString()} | Acid bought: ${TRACK.acidBought.toLocaleString()} | DMT bought: ${TRACK.dmtBought.toLocaleString()}`);
console.log(`  Used: ${TRACK.shroomsUsed.toLocaleString()} shrooms, ${TRACK.acidUsed.toLocaleString()} acid, ${TRACK.dmtUsed.toLocaleString()} DMT`);
console.log(`  Good trips: ${TRACK.goodTrips.toLocaleString()} | Bad trips: ${TRACK.badTrips.toLocaleString()} (3% rate, bunk eliminated)`);
