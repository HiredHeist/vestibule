...
export const ALL_CARDS=[
  ...
  {id:'herbmoney',name:'Herb Money',type:'RIFF',rarity:'Uncommon',emoji:'🌿',embers:1,effect:'Target member +3 ATK permanently. Drug money already bought the loudest amp.',color:'#9933cc',typeColor:'#7722aa',copies:1},
...
// Aug 9 2026 — EVERY card now has a real, JV-chosen upgrade (see FORGE_UPGRADES.md).
// No more "Gold foil / no rules change" placebos, and all 82 cards are forgeable.
// The mechanical effect lives in App.jsx handlers (live) + cardEngine.js IMPL (sim),
// both branching on `upgraded`. Keep desc text in sync with those two.
export const CARD_UPGRADES={
  // ── RIFF ──
  amp:{desc:'Double damage AND target +2 ATK permanently.'},
  battlecry:{desc:'+3 ATK permanently (was +1).'},
  newstrings:{desc:'+3 ATK permanently (was +2).'},
  encore:{desc:'Attacks twice AND +1 ATK permanently.'},
  demotape:{desc:'Replay last RIFF free, then draw 1.'},
  soundwall:{desc:'+2 ATK permanently to ALL (was +1).'},
  stagedive:{desc:'Deal 1.5× the target’s HP as damage.'},
  overdrive:{desc:'Only needs 40% Corruption (was 60%).'},
  infencore:{desc:'ALL attack twice, costs 2 Embers (was 3).'},
  possessedperf:{desc:'ALL ×3 ATK, costs 2 Embers (was 3).'},
  crowdsurf:{desc:'+1 extra ATK on top of the per-card bonus.'},
  heavyriff:{desc:'+4 extra ATK on top of the half-ATK bonus.'},
  resonancecard:{desc:'Becomes highest ATK on stage +2, this Strike.'},
  herbmoney:{desc:'+5 ATK permanently (was +3).'},
  goingbroke:{desc:'Deal 10× your Stash as damage.'},
  moshpit:{desc:'+2 ATK perm to all (+3 if 4+ alive).'},
  doubledown:{desc:'Next card 0 Embers, and draw 1 card.'},
  burnset:{desc:'Draw discarded +2 (was +1).'},
  echopedal:{desc:'Copy last card, next card free, draw 1.'},
  riffthief:{desc:'Copy last card, next free, draw 1.'},
  feedbackscream:{desc:'+4 ATK permanently, NO HP cost.'},
  skullsplitter:{desc:'+5 ATK permanently (no requirement).'},
  doomchord:{desc:'Adjacent +4 at just 30% Corruption (was 50%).'},
  bloodharmony:{desc:'+2 ATK permanently to the WHOLE band.'},
  sonicboom:{desc:'ALL +3 ATK this Strike (was +2), draw 1.'},
  tremolopick:{desc:'Counts itself toward the per-RIFF tally.'},
  harmonicfb:{desc:'+1 ATK perm per RIFF (min +1), and draw 1.'},
  shredsolo:{desc:'Attacks twice AND +2 ATK this Strike.'},
  overdriveped:{desc:'×1.5 multiplier, costs 1 Ember (was 2).'},
  devilsdice:{desc:'Only a 1 whiffs; 2-3 all +3, 4-6 all +5 +draw 2.'},
  necroticamp:{desc:'ALL +2 ATK this Strike per 20% Corruption.'},
  // ── CORRUPT ──
  distortion:{desc:'ALL +2 ATK this Strike (was +1).'},
  deathriff:{desc:'ALL +3 ATK permanently (was +2).'},
  feedbackloop:{desc:'+3 ATK perm (+5 at 50%+ Corruption).'},
  ampstatic:{desc:'+3 this Strike (+6 at 50%+ Corruption).'},
  darktuning:{desc:'Buffs the WHOLE band +1 ATK permanently.'},
  sigdecay:{desc:'Discard 1, draw 3 (was 2).'},
  controlfeedback:{desc:'Reset Corruption to 50%, fully heal the WHOLE band.'},
  sabbathsigil:{desc:'Same effect but the card is NOT destroyed.'},
  seance:{desc:'Heal all 3 (6 at 50%+), and revive Too Stoned.'},
  whispercard:{desc:'+3 ATK permanently (was +2).'},
  hungercard:{desc:'ALL +2 ATK permanently (was +1).'},
  madnesscard:{desc:'Deal 20% of boss max HP (was 15%).'},
  staticcharge:{desc:'Gain 2 Embers, or 6 if Corruption is 0%.'},
  soulbargain:{desc:'+5 ATK this Strike, +5% Corr, NO HP cost.'},
  venomriff:{desc:'+4 ATK permanently (was +3).'},
  offeringpit:{desc:'Choose the member (not random) for +8 ATK.'},
  cursedstrings:{desc:'+6 ATK this Strike, NO heal-lock.'},
  hexdecay:{desc:'Boss loses 20% of current HP (was 15%).'},
  infernalpact:{desc:'Set Corruption 66%, ALL +3 ATK perm (was +2).'},
  carrioncall:{desc:'Revive at HALF max HP with +5 ATK.'},
  possessionriff:{desc:'+20 ATK this Strike, NO Corruption cost.'},
  hellfirerift:{desc:'ALL ×2.5 ATK this Strike (was ×2).'},
  soulsacrifice:{desc:'+5 ATK to all perm, +10% Corruption (was 15%).'},
  voidpact:{desc:'×2.5 multiplier, +25% Corr, draw 2.'},
  darkcrescendo:{desc:'Triples strike multiplier at 60%+ Corruption (was 80%).'},
  russianroulette:{desc:'2-5 give +6, 6 gives +10 +Shield (1 still stones).'},
  drainthecrowd:{desc:'+1 ATK per 8% band HP missing (was 10%).'},
  corrsiphon:{desc:'+2 ATK per 10% Corruption (was +1), this Strike.'},
  bloodritual:{desc:'8× sacrificed HP as damage (was 6×).'},
  // ── UTILITY ──
  soundcheck:{desc:'ALL +6 HP (was +4), hurt members +2 ATK.'},
  roadie:{desc:'+4 HP, Stonewall shield 3 strikes (was 2).'},
  wakeup:{desc:'ALL +4 HP (was +2), revives KO’d.'},
  setbreak:{desc:'Discard 1, +3 Embers, -25% Corruption (was -15%).'},
  remaster:{desc:'Delete 1, draw 4 (was 3).'},
  setlist:{desc:'Draw up to 4 (was 3).'},
  gearcheck:{desc:'+8% per distinct card, costs 0 Embers (was 1).'},
  setlistrewrite:{desc:'Look top 3, discard costliest, keep 2, draw 1.'},
  backstagepass:{desc:'Next TWO cards free, draw 1.'},
  venueswap:{desc:'Draw 6 fresh cards, and gain 2 Embers.'},
  doublebooking:{desc:'+1 extra Strike, costs 2 Embers (was 3).'},
  bootlegcopy:{desc:'The copy goes to your HAND now (not deck).'},
  // ── EMBER ──
  powertap:{desc:'Gain 3 Embers (was 2).'},
  tappedout:{desc:'Gain 5 Embers immediately (no delay).'},
  ampoverload:{desc:'+3 Embers, no discard cost.'},
  groupie:{desc:'+3 Embers (was +2), draw 1.'},
  soundboard:{desc:'Gain 2 Embers, draw 1 now + 1 extra next Strike.'},
  secondwind:{desc:'Refill Embers to max, draw 2 (was 1).'},
  pyromaniac:{desc:'+3 Embers (was +2); spend all → all +3 ATK.'},
  slowburn:{desc:'+2 now (+2 next 2 Strikes), and draw 1 now.'},
  ampfeedback:{desc:'+2 Embers, next RIFF is FREE (was 1 less).'},
