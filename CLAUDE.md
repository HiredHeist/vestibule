...
- **Skill-Pass R&D flags (Aug 5, ALL env-gated, default-OFF — see TODO "SKILL-BASED REBALANCE"):** `SKILL_PASS=1` enables the experimental anti-spam ruleset; `LAZY=1` runs a random spam-player (for measuring the skill gap); `SP_EMBER_GEN_CAP=N` bounds total per-fight ember generation (leak-plug); `SP_RAW_DMG=F` scales raw band damage so chains dominate; plus `SP_EMBER_PER_STRIKE, SP_MULT_CAP, SP_CAP_ALL, SP_STRIKE_CAP_PCT, SP_THRESH, SP_CORR_PER_CARD`. With all flags off the sim is byte-identical to baseline. `decideDescentSkips` now returns `[]` (never skip — skipping fight 1 forfeited the free Welcome pack; this is a permanent default change, not gated). **The sim's greedy bot can't build combos, so it CANNOT validate chains-dominant balance — use the live bot for that.**
