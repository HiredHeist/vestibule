...
## 🐞 AUDIT FINDINGS (Aug 12 static pass) — several FIXED (⚠ need build-verify), rest queued
Screen audit + phantom-victory forensic. Status per item:
- [ ] **HIGH — Daily Challenge launches a random, untracked run. (DEFERRED to tomorrow w/ JV.)** EndScreen `onDailyChallenge` (~12125) + menu button (~11589) call `handleReset()` with no arg, so the PER_RUN reset overwrites the daily seed and `setIsDailyRun(false)` clobbers the daily flag → non-deterministic, untracked daily. Fix: thread daily ctx through `handleReset({seed:getDailySeed(),daily:true})`, honor in `_opts.seed`, `isDailyRun:o=>setIsDailyRun(!!o?.daily)`.
- [x] **HIGH — ANCHOR "dead aura" → NOT A BUG (resolved Aug 12).** ANCHOR's real effect — the per-fight lethal-hit save (`_tryAnchorSave` ~5650, tier-scaled, resets per fight) — is fully live. `_anchorAuraRed()` returning 0 is the *intentionally-removed* neighbor aura (neighbor auras were dropped this session, Mentor Link kept); the call sites are harmless no-ops. Optional tidy: strip the stub + its 3 call sites.
- [x] **MED — phantom victory (Aug 1 forensic) → FIXED (Aug 12, ⚠ build-verify).** Added the single choke-point guard `if(enemyHpRef.current>0)return` at the top of `triggerVictory` (after the [VICTORY] log, before the Lucifer intercept). Safe: enemyHpRef is synced via the [enemyHp] effect (~5470) so every real kill leaves it ≤0 by fire time, and dev Shift+W bypasses triggerVictory. Neutralizes all ~15 direct-damage kill paths at once. (Left the Stage-Dive double-schedule — harmless under victoryFiredRef; note for later.)
- [x] **MED — boss telegraph under-reports → FIXED (Aug 12, ⚠ build-verify).** Telegraph now applies Hunter ×1.5 / Executioner ×2 (`targetHighestHp2/3`) with a "×N vs strongest" tag, and replaced the flat +3-at-100% with the engine's true corruption multiplier `×(1+0.60·corr/100)` (CORR_DMG_TAKEN, no-op off Ritualist).
- [ ] **MED — cursed_amp "corruption locks forever" barely functions** — the `corruption_locked` flag is only honored by the corruption_engine pact (~10219); card plays / boss passives / events all ignore it (~6783/9826/10403). NOT YET FIXED (bigger enforcement change; wasn't in tonight's fix set). Enforce on the main `setCorruption` paths or drop the promise from the text.
- [x] **LOW — cosmetic/edge → FIXED (Aug 12, ⚠ build-verify):** hellfire_baptism log now says "+2 ATK" (matches grant); sabbath_offering now skips downed members (`!m.tooStoned`); EndScreen `circleReached` clamped to `Math.min(9,…)`. (EndScreen `saveDailyBest`-during-render deferred with the Daily Challenge work above.)
