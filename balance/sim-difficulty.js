...
// chainlab: resolved MODULE-relative so the lab runs from any cwd.
const _REPO_ROOT=new URL('../',import.meta.url).pathname
try{BOSS_HP_OVERRIDE=JSON.parse(readFileSync(_REPO_ROOT+'boss_hp_override.json','utf8'))}catch(e){
  try{BOSS_HP_OVERRIDE=JSON.parse(readFileSync('/tmp/boss_hp_override.json','utf8'))}catch(e2){}
}

// ═══════════════════════════════════════════════════════════════════════════
// CHAIN BALANCE LAB — experiment knobs (all read from env, all default to the
// SHIPPED behaviour so `LAB=... node balance/sim-chainlab.js` with no env vars
// reproduces vestibule-sim-kwstacks.js exactly).
// ═══════════════════════════════════════════════════════════════════════════
const LAB={
  // Multiplier applied per chain. Shipped = 1.78 flat, unlimited, per strike.
  chainMult:      parseFloat(process.env.CHAIN_MULT||'1.78'),
  // 'flat'  — every chain in a strike is worth chainMult (SHIPPED)
  // 'dim'   — diminishing per chain WITHIN a strike, from CHAIN_DIM ladder
  // 'add'   — chain adds flat damage scaled off band ATK instead of multiplying
  // 'none'  — chains give no damage at all (upper bound control)
  chainModel:     process.env.CHAIN_MODEL||'flat',
  // Diminishing ladder for chainModel='dim'. Last entry repeats for 4th+.
  chainDim:      (process.env.CHAIN_DIM||'1.78,1.40,1.20,1.05').split(',').map(Number),
  // 'strike' (SHIPPED) — chain id may re-fire every strike
  // 'fight'            — chain id fires at most once per FIGHT
  chainScope:     process.env.CHAIN_SCOPE||'strike',
  // Additive model: bonus damage = ADD_PCT × raw band ATK, per chain.
  chainAddPct:    parseFloat(process.env.CHAIN_ADD_PCT||'1.0'),
  // Where the additive lands. 'pre' (default) = before the multiplier stack, so
  // the bonus is itself amplified. 'post' = after everything, so a chain is a
  // flat ATK-proportional payoff that CANNOT compound.
  chainAddWhen:   process.env.CHAIN_ADD_WHEN||'pre',
  // Soft cap on the TOTAL strike multiplier. 0 = off (SHIPPED).
  // Above the knee the curve becomes knee * (1 + ln(m/knee) * SOFTCAP_K).
  softcapKnee:    parseFloat(process.env.SOFTCAP_KNEE||'0'),
  softcapK:       parseFloat(process.env.SOFTCAP_K||'1.0'),
  // ── VARIANT G: soft cap on TOTAL AMPLIFICATION (final strike damage measured
  // in units of raw band ATK), not just on strikeMult. strikeMult is only part
  // of the stack — Possessed Perf x3, Infernal Encore x2, corruption x3 and the
  // relic multipliers all sit outside it, which is why capping strikeMult alone
  // barely moves the one-shot rate. 0 = off.
  globalKnee:     parseFloat(process.env.GLOBAL_KNEE||'0'),
  globalK:        parseFloat(process.env.GLOBAL_K||'1.0'),
  // Chains cost embers / corruption to fire. 0 = free (SHIPPED).
  // If the player cannot pay the ember cost the chain simply does not fire.
  chainEmberCost: parseFloat(process.env.CHAIN_EMBER_COST||'0'),
  chainCorrCost:  parseFloat(process.env.CHAIN_CORR_COST||'0'),
  // Band base-ATK scale — the SENSITIVITY knob. 1 = shipped roster.
  atkScale:       parseFloat(process.env.ATK_SCALE||'1'),
  // POLICY knob. The shipped sim's card AI chains far less than the live bot
  // (0.26 chains/strike vs the bot's 1.9). chainSeek scales the AI's chain
  // priority so the lab can be run under a "good policy" calibration too.
  chainSeek:      parseFloat(process.env.CHAIN_SEEK||'1'),
  // Ember floor the AI keeps in reserve for a chain (used with CHAIN_EMBER_COST).
  minScore:       parseFloat(process.env.MIN_SCORE||'3'),
  // ── GOOD-POLICY CALIBRATION KNOBS ──
  // The shipped sim's AI sees 5 cards and plays ~4/strike; the live bot fires
  // ~7x more chains than that AI does. These two knobs widen how much of the
  // deck the AI can see and pay for per strike, which is the only lever in the
  // sim that moves the chain rate at all. They are a PROXY for "a good player /
  // the live bot", not a derived model — see CHAIN_BALANCE.md.
  handBonus:      parseInt(process.env.HAND_BONUS||'0'),
  emberRefill:    parseInt(process.env.EMBER_REFILL||'0'),
  // DECK_THIN=<n>: trim the starting deck to n cards, keeping chain partners
  // first (balanced across pairs). Models a player who drafted and BURNED for
  // chains — the single biggest driver of chain frequency, and the only knob
  // that moves the sim anywhere near the live bot's observed rate.
  deckThin:       parseInt(process.env.DECK_THIN||'0'),
  // CHAIN_FORCE=<rate>: top the strike up to a Poisson(rate) number of chains
  // regardless of what the AI managed to draw. This is the LIVE-PARITY regime:
  // the live bot fires ~1.9 chains/strike and no sim policy gets past ~0.67, so
  // this knob holds chain FREQUENCY at the live-measured value and lets the
  // multiplier MODEL be compared on equal footing. Phantom chains cost nothing
  // and ignore CHAIN_SCOPE (they are frequency, not identity).
  chainForce:     parseFloat(process.env.CHAIN_FORCE||'0'),
  // Output mode
  quiet:          process.env.QUIET==='1',
  label:          process.env.LAB_LABEL||'baseline',
  seed:           process.env.LAB_SEED?parseInt(process.env.LAB_SEED):0,
}

// ═══════════════════════════════════════════════════════════════════════════
// DIFFICULTY LAB — the difficulty-curve variants (V1/V5/V6/V8/V9/V11/V20).
// Every knob defaults to SHIPPED behaviour, so with no env vars set this file
// reproduces sim-chainlab.js / vestibule-sim-kwstacks.js exactly.
//
// DESIGN CONSTRAINT (JV): every number here must be FIXED, PUBLISHED and
// PLANNABLE. Nothing below reads live band power or rolls difficulty.
// ═══════════════════════════════════════════════════════════════════════════
const DIFF={
  // ── V1: NO OVERTIME. Shipped lets a fight run maxStrikes+8 ("the boss
  // ENRAGES"), so missing the HP number is not a loss — the fight becomes an
  // attrition grind. V1=1 hard-stops at maxStrikes: kill it or the run ends.
  v1NoOvertime:   process.env.V1_NO_OVERTIME==='1',
  // ── V5: BAND HP IS NOT A LOSS CONDITION. A Too Stoned member contributes no
  // ATK for the rest of the fight (so it costs output against the target) but
  // an all-stoned band no longer ends the run. Stoned-ness clears at the fight
  // boundary, restoring the member at V5_WAKE_PCT of max HP (default 25%), so
  // HP attrition still exists — it just cannot kill you. V6 removes the
  // attrition as well; the two are deliberately separable.
  v5HpNotLoss:    process.env.V5_HP_NOT_LOSS==='1',
  v5WakePct:      parseFloat(process.env.V5_WAKE_PCT||'0.25'),
  // ── V6: FULL HEAL BETWEEN EVERY FIGHT. No damage carries between fights.
  v6FullHeal:     process.env.V6_FULL_HEAL==='1',
  // ── V8: FIXED DRUMMER MULTIPLIER. Shipped rolls one d6 per FIGHT and
  // multiplies the whole band by 1.0 / 1.5 / 2.0 (EV 1.5, but a 2:1 swing of
  // pure noise on every fight). V8 replaces it with a fixed multiplier read off
  // the drummer keyword-stack tier: tier 1 / 2 / 3 -> V8_DRUM values.
  // Default 1.5/1.75/2.0 keeps tier 1 EV-neutral against the shipped d6.
  v8FixedDrum:    process.env.V8_FIXED_DRUM==='1',
  v8Drum:        (process.env.V8_DRUM||'1.5,1.75,2.0').split(',').map(Number),
  // ── V9 / V20: boss HP curve override. 'off' = shipped boss_hp_override.json.
  //   'v9'  — exponential published curve, tuned so raw band ATK alone cannot
  //           clear past ~Circle 4 and a multiplier engine becomes mandatory.
  //   'v20' — refit to MEASURED band output, targeting 3-4 strikes per fight
  //           and a steady death rate at every circle.
  //   'custom' — read HP_CURVE env (27 comma-separated base HP values).
  hpCurve:        process.env.HP_CURVE_MODE||'off',
  hpCurveCustom: (process.env.HP_CURVE||'').split(',').filter(Boolean).map(Number),
  // Global multiplier applied on top of the chosen curve — the single tuning
  // dial used to fit a target win rate without changing the curve's SHAPE.
  hpCurveMult:    parseFloat(process.env.HP_CURVE_MULT||'1'),
  hpCurveBossMult:parseFloat(process.env.HP_CURVE_BOSS_MULT||'1'),
  // ── V11: CAPPED PERMANENT ATK GROWTH on a fixed published per-fight
  // schedule. Band raw ATK (base + permanent bonuses, summed over the stage)
  // is clamped to CAP[fightIndex] at fight start; excess permanent ATK is
  // scaled back proportionally across members. Fixed table, no runtime read.
  v11AtkCap:      process.env.V11_ATK_CAP==='1',
  v11CapMult:     parseFloat(process.env.V11_CAP_MULT||'1'),
  // ── LUCIFER TOTAL HP. Live (App.jsx getScaledMaxHp ~5209) is a FLAT 666,666
  // total / 333,333 per phase, with NO deck hpScale and no stake multiplier —
  // Lucifer is special-cased and enemies.js maxHp:100000 is dead data for him.
  // 0 = use the live 666,666. Any other value is a TOTAL (split across the two
  // phases), which is how a variant must express a Lucifer change, because
  // shipping one means patching that special case, not enemies.js.
  luciferTotal:   parseFloat(process.env.LUCIFER_TOTAL||'0'),
  // ── POLICY: 'good' = shipped scoreCard AI. 'weak' = plays a RANDOM playable
  // card every time and never seeks chains. The strong/weak win-rate ratio is
  // the deck-skill sensitivity metric.
  policy:         process.env.POLICY||'good',
}
// V11 published ATK ceiling, indexed by fight 0..26. Fixed, printable table.
// Fitted from the measured winner band-ATK line (f0 17 / f1 35 / f2 62 / f3 84
// / f4 122 / f5 138 / f6 165 / f7 212) with headroom, then continued on the
// same growth rate. This is a CEILING, not a grant.
const V11_ATK_SCHEDULE=[
  20, 40, 70, 95, 130, 155, 185, 230, 270,
  310, 355, 405, 460, 520, 585, 655, 730, 810,
  895, 985, 1080, 1180, 1285, 1395, 1510, 1630, 1755,
].map(v=>Math.round(v*DIFF.v11CapMult))

// ── V9: EXPONENTIAL PUBLISHED HP CURVE ──
// FORMULA (base HP, before the deck hpScale the live game already applies):
//
//     HP(f) = round( 45 * 1.25^f )        f = fight index 0..26
//
// Two published constants: the Wanderer keeps its current 45, and every fight
// after it is 25% harder than the one before (x1.95 per circle).
//
// WHY 1.25: measured band RAW ATK grows at ~1.19x per fight. A curve growing
// at 1.25 outruns it by 1.05x per fight, so "4 strikes x raw band ATK" covers
// the number through fight 10 and falls permanently behind from fight 11 —
// Circle IV. From there the run is only clearable with a multiplier engine,
// which is the stated design requirement.
const V9_CURVE=Array.from({length:27},(_,f)=>Math.round(45*Math.pow(1.25,f)))

// ── V20: HP CURVE REFIT TO MEASURED BAND OUTPUT ──
// Not a one-line formula: measured band output is NOT a clean exponential
// (the band triples in size during Circle I, then growth decelerates), so a
// single exponential misses by 5x at fight 0. This is a published 27-entry
// table, generated by a stated rule:
//
//   1. measure median strike damage at each fight index (balance/sim-difficulty)
//   2. target HP_effective(f) = 3.5 x medianStrikeDamage(f)
//   3. divide out the live scaling (deck hpScale 1.85 x stake 1.20/1.30) to
//      get the base number that goes in enemies.js
//   4. smooth ln(target) with a 3-point centred mean, force monotonic
//   5. multiply circle bosses (every 3rd fight) by 1.15 — the boss is the check
//   6. round to 2 significant figures so the numbers are memorable
//
// Result: 3.0-4.7 strikes at median damage at EVERY fight, against the shipped
// curve's 1.6-23.8 spread.
const V20_CURVE=[
  30,    74,   180,    // Circle I    — Wanderer / Lost Soul / Drifter
  270,  410,   660,    // Circle II   — Siren / Tempter / Seducer
  770,  900,  1300,    // Circle III  — Glutton / Feaster / Devourer
 1300, 1600,  2400,    // Circle IV   — Miser / Hoarder / Usurer
 2300, 2700,  3600,    // Circle V    — Wrathful / Berserker / Warlord
 3800, 4600,  6000,    // Circle VI   — Heretic / Apostate / False Prophet
 5600, 6100,  8100,    // Circle VII  — Brute / Hunter / Executioner
 8100, 9700, 11000,    // Circle VIII — Trickster / Deceiver / Archfraud
11000,12000, 30000,    // Circle IX   — Traitor / Betrayer / Lucifer
]

// ── V20F: V20 REFIT FOR A WORLD WITH NO OVERTIME ("flat check") ──
// V20 above targets 3.5 strikes at MEDIAN output. That is the right target when
// a fight can run long, and the wrong one when it cannot: HP at the median of
// 4-strike output means each fight is a coin flip, and 27 coin flips is a 0%
// win rate. A 22% run win rate over 27 sequential hard checks needs
//     0.22 ^ (1/27) = 94.5% pass on EVERY fight,
// i.e. the number must sit near the 5th percentile of 4-strike output.
// V20F is that curve, produced by balance/fit-curve.mjs iterating until every
// fight index has the same measured pass rate (converged: RMSE 0.008).
const V20F_CURVE=[25,58,110,270,430,550,880,1000,1100,1400,1700,2000,3700,2700,3200,4100,4900,5700,6200,7000,7700,10000,11000,12000,10000,16000,55000]

// ── V20B: the same refit, BALATRO-SHAPED ──
// Same total difficulty, redistributed: the two non-boss fights of each circle
// are a formality (fitted to a 98% pass rate) and the CIRCLE BOSS is the check
// (fitted to 90%). Solves p_nonboss^18 * p_boss^9 = 0.22.
const V20C_CURVE=[17,41,110,170,260,530,550,590,1000,890,1100,1900,2300,1600,3000,2600,2900,5300,3700,4000,7000,6000,6500,11000,7200,10000,72000]

function diffCurveHp(f){
  let base
  if(DIFF.hpCurve==='v9')base=V9_CURVE[f]
  else if(DIFF.hpCurve==='v20')base=V20_CURVE[f]
  else if(DIFF.hpCurve==='v20f')base=V20F_CURVE[f]
  else if(DIFF.hpCurve==='v20c'||DIFF.hpCurve==='v20b')base=V20C_CURVE[f]
  else if(DIFF.hpCurve==='custom')base=DIFF.hpCurveCustom[f]
  else return null
  if(base===undefined||!isFinite(base))return null
  // Boss fights (every 3rd) get their own dial so "comfortable trash / tense
  // boss" can be tuned without changing the curve's overall level.
  const _bm=((f+1)%3===0)?DIFF.hpCurveBossMult:1
  return Math.max(1,Math.round(base*DIFF.hpCurveMult*_bm))
}
// Deterministic RNG so variant A vs variant B is compared on the same runs.
// Shipped sim uses Math.random(); when LAB_SEED is set we swap in a seeded
// mulberry32 so the variant grid is paired rather than independent samples.
if(LAB.seed){
  let _s=LAB.seed>>>0
  Math.random=function(){_s|=0;_s=_s+0x6D2B79F5|0;let t=Math.imul(_s^_s>>>15,1|_s);t=t+Math.imul(t^t>>>7,61|t)^t;return((t^t>>>14)>>>0)/4294967296}
}
// Per-strike instrumentation. One record per strike resolved.
const LABSTATS={
  strikes:0, chainFires:0, chainsBlockedCost:0,
  chainsPerStrike:{},          // histogram
  amp:[],                      // finalStrikeDmg / rawBandAtk, NON-LETHAL strikes only
  totalMult:[],                // gs._strikeMult at resolution
  rawAtkSum:0, finalDmgSum:0,  // for "how much of damage is band ATK"
  chainMultProduct:[],         // product of just the chain multipliers this strike
  cardsPlayedSum:0,
  bossFights:0,bossStrikes:0,bossOneShots:0,
  // per-fight-index breakdown: does the sim reproduce the live bot's 1.9
  // chains/strike once a run reaches the depth the bot's runs reached?
  byFight:Array.from({length:27},()=>({strikes:0,chains:0,cards:0,amp:[]})),
}
// ── DIFFICULTY LAB instrumentation ──
const DIFFSTATS={
  strikesHist:{},              // strikes-used -> count of fights
  fightsReachedMaxStrike:0,    // fights that needed the 4th (final allowed) strike
  fightsFought:0,
  bandAtkByFight:Array.from({length:27},()=>[]),   // raw band ATK, first strike of each fight
  dmgByFight:Array.from({length:27},()=>[]),       // strike damage dealt, non-lethal strikes
  strikesByFight:Array.from({length:27},()=>[]),
  deathCause:{stoned:0,beaten:0},
  deathCauseByCircle:Array.from({length:10},()=>({stoned:0,beaten:0})),
  fightsLostOnNumber:0,        // V1: fights where the allowance ran out with boss alive
  bossFights:0,bossFinalStrike:0,bossStrikesHist:{},  // circle-boss-only pace
  overtimeFights:0,            // fights that went past the strike allowance (shipped)
  overtimeStrikes:0,
  reachedFight:new Array(28).fill(0),  // runs that STARTED fight f
  fightsWonAt:new Array(27).fill(0),   // fights CLEARED at index f
  fightsFoughtAt:new Array(27).fill(0),
  wonAfterReaching:new Array(28).fill(0),
}
function diffPct(arr,p){if(!arr.length)return 0;const a=arr.slice().sort((x,y)=>x-y);return a[Math.min(a.length-1,Math.floor(p*a.length))]}
function labPct(arr,p){if(!arr.length)return 0;const a=arr.slice().sort((x,y)=>x-y);return a[Math.min(a.length-1,Math.floor(p*a.length))]}
// Soft cap transform, applied to the TOTAL strike multiplier at resolution.
// Apply ONE chain's payoff under the active model.
function labApplyChain(gs){
  const _nth=(gs._chainsThisStrike||0)   // 0-based index within this strike
  gs._chainsThisStrike=_nth+1
  // 'add' and 'hybrid' bank an ATK-scaled additive, applied at strike
  // resolution where raw band ATK is known.
  if(LAB.chainModel==='add'||LAB.chainModel==='hybrid')gs._chainAddCount=(gs._chainAddCount||0)+1
  // 'flat' / 'dim' / 'hybrid' also multiply the strike.
  if(LAB.chainModel==='flat'||LAB.chainModel==='dim'||LAB.chainModel==='hybrid'){
    let f=LAB.chainMult
    if(LAB.chainModel==='dim'||LAB.chainModel==='hybrid'){const L=LAB.chainDim;f=L[Math.min(_nth,L.length-1)]}
    gs._chainMultProduct=(gs._chainMultProduct||1)*f
    gs._strikeMult=Math.min(10000,Math.round((gs._strikeMult*f)*100)/100)
  }
}
function labPoisson(lam){let L=Math.exp(-lam),k=0,p=1;do{k++;p*=Math.random()}while(p>L);return k-1}
function labSoftcap(m){
  if(!LAB.softcapKnee||m<=LAB.softcapKnee)return m
  return LAB.softcapKnee*(1+Math.log(m/LAB.softcapKnee)*LAB.softcapK)
}
...
export function assertBossHpSync(overrideJson,enemyList){
  ...
  // LUCIFER IS EXEMPT — ported verbatim from vestibule-sim-kwstacks.js:49.
  ...
  // maxHp:100000 in enemies.js is DEAD DATA for Lucifer.
...
function makeMember(base,foil,mythic,demonic,isStarter){
  ...
  // ── CHAINLAB: band base-ATK sensitivity knob. Scales the ROSTER's ATK only —
  // card-granted flat +ATK is untouched, so this measures "does a better band
  // matter", not "does everything scale".
  if(LAB.atkScale!==1)m.atk=Math.max(1,Math.round(m.atk*LAB.atkScale))
  return m;
}
function buildDeck(){const d=[];for(const[id,copies]of Object.entries(ACTIVE_DECK)){const card=ALL_CARDS.find(c=>c.id===id);if(card)for(let i=0;i<copies;i++)d.push({...card,uid:Math.random().toString(36).slice(2)})}
  if(LAB.deckThin>0&&d.length>LAB.deckThin){
    const present=new Set(d.map(c=>c.id))
    const partnered=new Set()
    for(const ch of RIFF_CHAINS_SIM)if(present.has(ch[0])&&present.has(ch[1])){partnered.add(ch[0]);partnered.add(ch[1])}
    const keep=[],rest=[]
    for(const c of shuffle(d))(partnered.has(c.id)?keep:rest).push(c)
    return shuffle(keep.concat(rest).slice(0,LAB.deckThin))
  }
  return shuffle(d)}
// DIFF: the shipped sim drafts the BEST of 40 random starting pairs — a perfect
// draft the live player/bot does not get, which is why the sim's fight-0 band
// ATK is ~24 against the live bot's ~15 (and 4 in runs that die there).
// START_PICK=<n> = best of n candidate pairs. 1 = a random pair.
function pickStartingPair(){const pool=ALL_MUSICIANS.filter(m=>!m.locked);let best=null,bs=-1;const _tries=parseInt(process.env.START_PICK||'40');let _valid=0;for(let i=0;i<2000&&_valid<_tries;i++){const a=pick(pool),b=pick(pool);if(a.id===b.id)continue;if(a.keyword==='ANCHOR'&&b.keyword==='ANCHOR')continue;_valid++;const s=memberScore(a)+memberScore(b);if(s>bs){bs=s;best=[a,b]}}return best.map(b=>makeMember(b,false,false,false,true))}
...
function decideDescentSkips(gs,circleNum){
  const skips=[]
  // DIFF: the shipped sim ALWAYS skips fight 0 of Circle 1, so it never fights
  // The Wanderer at all — while the live bot fights it and dies there 47% of the
  // time. NO_SKIP=1 turns descent skipping off so the sim's death distribution is
  // comparable to the live measurement (and so V1 is tested against fight 0).
  if(process.env.NO_SKIP==='1')return skips
...
function scoreCard(card,gs,enemy,strikeNum,cardsPlayed){
  ...
  for(const chain of RIFF_CHAINS_SIM){
    ...
    if(partner&&gs.hand.some(c=>c.id===partner)){base+=15*LAB.chainSeek;break}
  }
  return base;
}
...
function simFight(gs,phaseHp,luciferPhase){
  ...
  let effectiveMaxHp
  const _diffHp=diffCurveHp(fightIdx)
  if(_diffHp!==null&&!gs._wthFight){
    // V9 / V20: published curve replaces the per-boss table. Deck hpScale and
    // the stake multiplier still apply exactly as live's getScaledMaxHp does,
    // so this is a drop-in replacement for enemies.js maxHp values.
    effectiveMaxHp=phaseHp||Math.ceil(_diffHp*HP_SCALE*(STAKE.hpMult/1.30))
  } else if(BOSS_HP_OVERRIDE&&BOSS_HP_OVERRIDE[fightIdx]){
  ...
  // CHAINLAB per-fight chain state
  gs._firedChainsFight=new Set();gs._chainsThisStrike=0;gs._chainAddCount=0;gs._chainMultProduct=1
  ...
  let _bandDbl=!_hasDrummer?1:(_dblRoll<=2?1.0:_dblRoll<=4?1.5:2.0)
  // ── V8: FIXED DRUMMER MULTIPLIER (replaces the d6 above) ──
  // Published, plannable: count drummers on stage (foil counts double, same as
  // every other keyword stack), take the stack tier, read the fixed multiplier.
  if(DIFF.v8FixedDrum){
    const _drumCount=gs.stage.filter(m=>m&&!m.tooStoned&&m.role==='Drummer').reduce((s,m)=>s+(m.foil?2:1),0)
    const _tier=stackTier(_drumCount)   // 0 = none, 1..3
    _bandDbl=_tier<=0?1:DIFF.v8Drum[Math.min(DIFF.v8Drum.length-1,_tier-1)]
  }
  // ── V11: CAPPED PERMANENT ATK GROWTH, fixed published schedule ──
  // Clamp the band's raw ATK line to the published ceiling for this fight by
  // scaling back permanent bonuses proportionally. Base member ATK is never
  // reduced below itself; only the accumulated permAtkBonus is trimmed.
  if(DIFF.v11AtkCap){
    const _cap=V11_ATK_SCHEDULE[Math.min(26,fightIdx)]
    const _live=gs.stage.filter(m=>m&&!m.tooStoned)
    const _tot=_live.reduce((s,m)=>s+m.atk+(m.permAtkBonus||0),0)
    if(_tot>_cap){
      const _permTot=_live.reduce((s,m)=>s+Math.max(0,m.permAtkBonus||0),0)
      const _excess=_tot-_cap
      if(_permTot>0){
        const _keep=Math.max(0,(_permTot-_excess)/_permTot)
        _live.forEach(m=>{if((m.permAtkBonus||0)>0)m.permAtkBonus=Math.floor(m.permAtkBonus*_keep)})
      }
    }
  }
  const dtMult={}
  ...
  // ── V1: NO OVERTIME — the strike allowance is the check. Shipped adds +8.
  const _otBudget=DIFF.v1NoOvertime?0:8
  for(let strike=0;strike<gs._fightMaxStrikes+_otBudget;strike++){
    const _otLevel=Math.max(0,strike-gs._fightMaxStrikes+1)
    if(strike>=gs._fightMaxStrikes){DIFFSTATS.overtimeStrikes++;if(strike===gs._fightMaxStrikes)DIFFSTATS.overtimeFights++}
    ...
    const handSize=_baseHand+LAB.handBonus+(gs._speedDemon?1:0)+(gs._drawNextStrike||0);
    if(LAB.emberRefill>0)gs.embers=Math.min(gs.maxEmbers,gs.embers+LAB.emberRefill);
    ...
    // V5: an all-stoned band no longer ends the fight — it just deals no ATK.
    // The fight runs its full allowance and is lost by MISSING THE NUMBER.
    const alive=gs.stage.filter(m=>!m.tooStoned);if(alive.length===0&&!DIFF.v5HpNotLoss)break;
    ...
    for(let att=0;att<15;att++){
      ...
      let best=playable[0];if(best.score<=LAB.minScore)break;
      // ── DECK-SKILL SENSITIVITY: the WEAK policy plays a random affordable
      // card instead of the best one, so it never sequences for a riff chain
      // and never holds embers for a payoff. Everything else is identical.
      if(DIFF.policy==='weak')best=playable[rand(playable.length)];
      const card=best.c
      ...
      // Riff chain detection — CHAINLAB configurable
      for(const chain of RIFF_CHAINS_SIM){
        if(gs._cardsPlayedIds.includes(chain[0])&&gs._cardsPlayedIds.includes(chain[1])){
          if(!gs._firedChains)gs._firedChains=new Set()
          const ck=chain[0]+'+'+chain[1]
          if(gs._firedChains.has(ck))continue
          // ── VARIANT B: once per chain id per FIGHT ──
          if(LAB.chainScope==='fight'){
            if(!gs._firedChainsFight)gs._firedChainsFight=new Set()
            if(gs._firedChainsFight.has(ck)){gs._firedChains.add(ck);continue}
          }
          // ── VARIANT F: chains cost something ──
          if(LAB.chainEmberCost>0&&gs.embers<LAB.chainEmberCost){
            LABSTATS.chainsBlockedCost++
            continue   // cannot afford — chain does not fire, may fire later this strike
          }
          gs._firedChains.add(ck)
          if(LAB.chainScope==='fight')gs._firedChainsFight.add(ck)
          if(LAB.chainEmberCost>0)gs.embers=Math.max(0,gs.embers-LAB.chainEmberCost)
          if(LAB.chainCorrCost>0)gs.corruption=Math.min(100,gs.corruption+LAB.chainCorrCost)
          labApplyChain(gs)
    ...
    // ── LIVE-PARITY CHAIN FREQUENCY (CHAIN_FORCE) ──
    if(LAB.chainForce>0&&(gs._cardsPlayedIds||[]).length>0){
      const want=labPoisson(LAB.chainForce)
      for(let _i=(gs._chainsThisStrike||0);_i<want;_i++){
        // Phantom chains still PAY (variant F must be measurable in this regime).
        if(LAB.chainEmberCost>0){
          if(gs.embers<LAB.chainEmberCost){LABSTATS.chainsBlockedCost++;break}
          gs.embers-=LAB.chainEmberCost
        }
        if(LAB.chainCorrCost>0)gs.corruption=Math.min(100,gs.corruption+LAB.chainCorrCost)
        labApplyChain(gs);LABSTATS.chainFires++;TRACK.combosTriggered=(TRACK.combosTriggered||0)+1
        if(DECK_ID_DEF.signature==='riff_chain_echo')gs._shredderEchoesPending=(gs._shredderEchoesPending||0)+1}
    }
    ...
    for(const m of aliveNow){
      ...
      // CHAINLAB: raw band ATK = the band's stat line, before any card-driven
      // per-strike multiplier (amp/possessed/overdrive/double-time) touches it.
      if(m.role!=='Drummer')_labRawBandAtk+=Math.max(0,atk)*((m.encoreThisStrike||m._kwDoubleStrike)?2:1)
    ...
    // ── VARIANT D: chains add flat damage scaled off raw band ATK ──
    // Enters BEFORE the multiplier stack, i.e. a chain makes the BAND hit harder
    // rather than multiplying whatever the card stack has already built.
    if((LAB.chainModel==='add'||LAB.chainModel==='hybrid')&&(gs._chainAddCount||0)>0&&LAB.chainAddWhen==='pre'){
      strikeDmg+=Math.round(_labRawBandAtk*LAB.chainAddPct*gs._chainAddCount)
    }
    ...
    // ── VARIANT E: soft cap the TOTAL strike multiplier ──
    const _labEffMult=labSoftcap(gs._strikeMult)
    if(_labEffMult>1.0)strikeDmg=Math.round(strikeDmg*_labEffMult)
    if((LAB.chainModel==='add'||LAB.chainModel==='hybrid')&&(gs._chainAddCount||0)>0&&LAB.chainAddWhen==='post'){
      strikeDmg+=Math.round(_labRawBandAtk*LAB.chainAddPct*gs._chainAddCount)
    }
    // ── VARIANT G: global amplification soft cap ──
    if(LAB.globalKnee>0&&_labRawBandAtk>0){
      const _amp=strikeDmg/_labRawBandAtk
      if(_amp>LAB.globalKnee){
        strikeDmg=Math.round(_labRawBandAtk*LAB.globalKnee*(1+Math.log(_amp/LAB.globalKnee)*LAB.globalK))
      }
    }
    // ── CHAINLAB instrumentation ──
    {
      LABSTATS.strikes++
      LABSTATS.cardsPlayedSum+=(gs._cardsPlayedIds||[]).length
      {const _bf=LABSTATS.byFight[Math.min(26,gs.fightIndex|0)]
       _bf.strikes++;_bf.chains+=(gs._chainsThisStrike||0);_bf.cards+=(gs._cardsPlayedIds||[]).length
       if(_labRawBandAtk>0&&strikeDmg<enemy._hp)_bf.amp.push(strikeDmg/_labRawBandAtk)}
      // DIFF: raw band ATK line on the FIRST strike of each fight — the "band
      // ATK by fight" series the live bot reports, measured the same way.
      if(strike===0)DIFFSTATS.bandAtkByFight[Math.min(26,gs.fightIndex|0)].push(_labRawBandAtk)
      // DIFF: actual strike damage by fight — the input to the V20 curve fit.
      // Sampled on non-lethal strikes only, so it is not truncated by boss HP.
      if(strikeDmg<enemy._hp)DIFFSTATS.dmgByFight[Math.min(26,gs.fightIndex|0)].push(strikeDmg)
      const _n=gs._chainsThisStrike||0
      LABSTATS.chainsPerStrike[_n]=(LABSTATS.chainsPerStrike[_n]||0)+1
      LABSTATS.totalMult.push(_labEffMult)
      LABSTATS.chainMultProduct.push(gs._chainMultProduct||1)
      LABSTATS.rawAtkSum+=_labRawBandAtk
      LABSTATS.finalDmgSum+=strikeDmg
      // Amplification over NON-LETHAL strikes only, matching the live-bot metric:
      // a lethal strike truncates against boss HP and biases the ratio.
      if(_labRawBandAtk>0&&strikeDmg<enemy._hp)LABSTATS.amp.push(strikeDmg/_labRawBandAtk)
    }
    ...
    gs._chainsThisStrike=0;gs._chainAddCount=0;gs._chainMultProduct=1
    if(gs.artifacts.some(a=>a.id==='a3'))gs._nextCardFree=true
    ...
    if(gs.stage.every(m=>m.tooStoned)&&!DIFF.v5HpNotLoss)break;
  }

  const won=enemy._hp<=0
  // V5: being wiped is never the loss reason. If the boss is alive at this
  // point the run ends because the NUMBER was missed, not because of HP.
  const allDead=DIFF.v5HpNotLoss?false:gs.stage.every(m=>m.tooStoned);
  ...
  // ── DIFFICULTY LAB: per-fight pace instrumentation ──
  {
    // The Welcome-to-Hell bonus fight runs at fightIndex 26 with its own boss
    // and is OPTIONAL — counting it would poison fight 26's pass rate (and did:
    // it dragged the fitted Lucifer HP down to a trivial number).
    if(!gs._wthFight){
      DIFFSTATS.fightsFought++
      DIFFSTATS.strikesHist[_strikesUsed]=(DIFFSTATS.strikesHist[_strikesUsed]||0)+1
      if(_strikesUsed>=gs._fightMaxStrikes)DIFFSTATS.fightsReachedMaxStrike++
      if(!won)DIFFSTATS.fightsLostOnNumber++
      if(fightIdx<27){DIFFSTATS.strikesByFight[fightIdx].push(_strikesUsed);DIFFSTATS.fightsFoughtAt[fightIdx]++;if(won)DIFFSTATS.fightsWonAt[fightIdx]++}
      // circle-boss-only pace. In a Balatro-shaped curve the two non-boss fights
      // are deliberately trivial, so an ALL-FIGHTS tension metric is diluted by
      // 18 formalities and says nothing about whether the checks are tense.
      if((fightIdx+1)%3===0){
        DIFFSTATS.bossFights++
        DIFFSTATS.bossStrikesHist[_strikesUsed]=(DIFFSTATS.bossStrikesHist[_strikesUsed]||0)+1
        if(_strikesUsed>=gs._fightMaxStrikes)DIFFSTATS.bossFinalStrike++
      }
    }
  }
  // CHAINLAB: boss-only pace. Trash fights (Wanderer, 45 HP) SHOULD be one-shot;
  // mixing them in makes the shipped one-shot rate look tamer than it is.
  if((gs.fightIndex+1)%3===0){LABSTATS.bossFights++;LABSTATS.bossStrikes+=_strikesUsed;if(won&&_strikesUsed<=1)LABSTATS.bossOneShots++}
  if(won){
    gs.fightsSurvived++;
  ...
  // ── V5 / V6: between-fight recovery ──
  // V5 wakes Too Stoned members at a fixed fraction of max HP (the ATK loss was
  // paid for that fight, and only that fight). V6 removes carried attrition
  // entirely by restoring everyone to full.
  if(won||DIFF.v5HpNotLoss||DIFF.v6FullHeal){
    if(DIFF.v6FullHeal){
      gs.stage.forEach(m=>{if(m){m.hp=m.maxHp;m.tooStoned=false;m.stoneShield=false}})
    } else if(DIFF.v5HpNotLoss){
      gs.stage.forEach(m=>{if(m&&m.tooStoned){m.tooStoned=false;m.hp=Math.max(1,Math.ceil(m.maxHp*DIFF.v5WakePct))}})
    }
  }
  return{won,allDead};
}
  ...
  if(evt==='mosh_pit'){
    ...
    if(Math.random()<0.25){
      if(Math.random()<0.5){
        ...
        // PRE-EXISTING SIM BUG (also in vestibule-sim-kwstacks.js:1341 and
        // sim-chainlab.js:1534): `alive` can be EMPTY here — a fight won on the
        // same strike that stones the last member leaves a fully-stoned band
        // alive into the between-fight event, and this reduce has no initial
        // value, so the whole run crashes with "Reduce of empty array".
        // Rate is ~1 in 90,000 games, which is why it has never been noticed;
        // it killed 2 of 46 cells in this experiment's first matrix run.
...
function simGame(){const gs=newGame();let deathFight=-1,deathCause='';
  for(let f=0;f<27;f++){gs.fightIndex=f;
    ...
    if(f===26){
      ...
      // ── LUCIFER IS A FLAT TOTAL, NOT A CURVE ENTRY ──────────────────────
      // App.jsx getScaledMaxHp (~5209) special-cases passiveId==='luciferBoss'
      // and returns a FLAT 333,333 per phase / 666,666 total: NO deck hpScale,
      // no stake multiplier, no boss-kill reduction. Six sites agree (5213,
      // 7174, 7188/7191, 9648/9653, 10265). enemies.js maxHp:100000 is DEAD
      // DATA for Lucifer — nothing reads it for the real fight, which is why
      // the sim's 666666 hardcode is CORRECT and always was.
      //
      // Consequence for this lab: any Lucifer number a variant wants must be
      // expressed as a TOTAL, unscaled, and shipped by patching the special
      // case in getScaledMaxHp — NOT by editing enemies.js, which would do
      // nothing. LUCIFER_TOTAL sets that total directly.
      // Entry 26 of a variant curve is therefore read as a FLAT TOTAL, not as a
      // base to be multiplied by hpScale like every other entry.
      const _curveLuci=(DIFF.hpCurve!=='off')?diffCurveHp(26):null
      const actualHp=DIFF.luciferTotal>0?DIFF.luciferTotal:(_curveLuci!==null?_curveLuci:666666)
...
for(let i=0;i<NUM_GAMES;i++){const r=simGame();totalFights+=r.fightsSurvived;totalPacts+=r.pacts;if(r.mentorLinks>0)gamesWithLinks++;
  if(r.won){wins++;if(r.wthWon)wthWins++;for(let f=0;f<27;f++)surviveByFight[f]++;for(let f=0;f<28;f++)DIFFSTATS.wonAfterReaching[f]++}
  else{for(let f=0;f<r.deathFight;f++)surviveByFight[f]++;deathsByFight[r.deathFight]++
    DIFFSTATS.deathCause[r.deathCause==='stoned'?'stoned':'beaten']++
    const _dc=Math.min(9,Math.floor(r.deathFight/3)+1)
    DIFFSTATS.deathCauseByCircle[_dc][r.deathCause==='stoned'?'stoned':'beaten']++}}
const elapsed=((Date.now()-t0)/1000).toFixed(1);

// ═══ CHAINLAB JSON SUMMARY ═══
{
  const cps=LABSTATS.chainsPerStrike
  const totS=Math.max(1,LABSTATS.strikes)
  const deathsByCircle={}
  for(let c=1;c<=9;c++){let d=0;for(let f=(c-1)*3;f<c*3;f++)d+=deathsByFight[f];deathsByCircle['C'+c]=+(d/NUM_GAMES*100).toFixed(2)}
  const out={
    label:LAB.label,games:NUM_GAMES,stake:STAKE_ID,deck:DECK_ID,seed:LAB.seed,
    cfg:{model:LAB.chainModel,mult:LAB.chainMult,dim:LAB.chainDim,scope:LAB.chainScope,addPct:LAB.chainAddPct,
         knee:LAB.softcapKnee,k:LAB.softcapK,emberCost:LAB.chainEmberCost,corrCost:LAB.chainCorrCost,atkScale:LAB.atkScale},
    winRate:+(wins/NUM_GAMES*100).toFixed(2),
    avgFightReached:+(totalFights/NUM_GAMES).toFixed(2),
    deathsByCircle,
    strikesPerFight:+(TRACK.strikesTaken/Math.max(1,TRACK.fightsFought)).toFixed(2),
    oneShotRate:+(TRACK.oneShotFights/Math.max(1,TRACK.fightsFought)*100).toFixed(1),
    bossOneShotRate:+(LABSTATS.bossOneShots/Math.max(1,LABSTATS.bossFights)*100).toFixed(1),
    bossStrikesPerFight:+(LABSTATS.bossStrikes/Math.max(1,LABSTATS.bossFights)).toFixed(2),
    chainsPerStrike:+(LABSTATS.chainFires/totS).toFixed(3),
    cardsPerStrike:+(LABSTATS.cardsPlayedSum/totS).toFixed(2),
    chainsPerFight:+(LABSTATS.chainFires/Math.max(1,TRACK.fightsFought)).toFixed(2),
    chainsBlockedCost:LABSTATS.chainsBlockedCost,
    chainHist:Object.fromEntries(Object.entries(cps).map(([k,v])=>[k,+(v/totS*100).toFixed(1)])),
    ampMedian:+labPct(LABSTATS.amp,0.50).toFixed(2),
    ampP90:+labPct(LABSTATS.amp,0.90).toFixed(2),
    ampP99:+labPct(LABSTATS.amp,0.99).toFixed(2),
    ampMax:+(LABSTATS.amp.reduce((a,b)=>b>a?b:a,0)).toFixed(1),   // NOT Math.max(...arr): blows the stack past ~120k samples
    ampN:LABSTATS.amp.length,
    multMedian:+labPct(LABSTATS.totalMult,0.50).toFixed(2),
    multP90:+labPct(LABSTATS.totalMult,0.90).toFixed(2),
    chainShareMedian:+labPct(LABSTATS.chainMultProduct,0.50).toFixed(2),
    chainShareP90:+labPct(LABSTATS.chainMultProduct,0.90).toFixed(2),
    // What fraction of total damage dealt is explained by the raw band stat line
    bandAtkShare:+(LABSTATS.rawAtkSum/Math.max(1,LABSTATS.finalDmgSum)*100).toFixed(3),
    seconds:+elapsed,
    byFight:LABSTATS.byFight.map((b,i)=>({f:i,strikes:b.strikes,chPerStrike:+(b.chains/Math.max(1,b.strikes)).toFixed(3),cardsPerStrike:+(b.cards/Math.max(1,b.strikes)).toFixed(2),ampMed:+labPct(b.amp,0.5).toFixed(2),ampP90:+labPct(b.amp,0.9).toFixed(2)})),
  }
  // ═══ DIFFICULTY LAB additions ═══
  {
    const G=NUM_GAMES
    const dbc=out.deathsByCircle
    out.diff={
      cfg:{v1:DIFF.v1NoOvertime,v5:DIFF.v5HpNotLoss,v6:DIFF.v6FullHeal,v8:DIFF.v8FixedDrum,
           v11:DIFF.v11AtkCap,hpCurve:DIFF.hpCurve,hpMult:DIFF.hpCurveMult,luciferTotal:DIFF.luciferTotal||666666,policy:DIFF.policy},
      // middle mass = share of ALL runs that end in circles 2..8
      middleMass:+([2,3,4,5,6,7,8].reduce((s,c)=>s+dbc['C'+c],0)).toFixed(2),
      c1Deaths:dbc.C1, c9Deaths:dbc.C9,
      // conditional win rates. reachedFight[f] counts runs that STARTED fight f.
      condWinAtF3:+(DIFFSTATS.reachedFight[3]?wins/DIFFSTATS.reachedFight[3]*100:0).toFixed(2),
      condWinPastF3:+(DIFFSTATS.reachedFight[4]?wins/DIFFSTATS.reachedFight[4]*100:0).toFixed(2),
      condWinAtF1:+(DIFFSTATS.reachedFight[1]?wins/DIFFSTATS.reachedFight[1]*100:0).toFixed(2),
      reachRate:Object.fromEntries([1,2,3,4,6,9,12,15,18,21,24,26].map(f=>['f'+f,+(DIFFSTATS.reachedFight[f]/G*100).toFixed(1)])),
      // pace
      strikesMedian:diffPct(Object.entries(DIFFSTATS.strikesHist).flatMap(([k,v])=>new Array(v).fill(+k)),0.5),
      strikesHist:Object.fromEntries(Object.entries(DIFFSTATS.strikesHist).sort((a,b)=>a[0]-b[0]).map(([k,v])=>[k,+(v/Math.max(1,DIFFSTATS.fightsFought)*100).toFixed(1)])),
      pctFightsToFinalStrike:+(DIFFSTATS.fightsReachedMaxStrike/Math.max(1,DIFFSTATS.fightsFought)*100).toFixed(1),
      pctBossFightsToFinalStrike:+(DIFFSTATS.bossFinalStrike/Math.max(1,DIFFSTATS.bossFights)*100).toFixed(1),
      bossStrikesHist:Object.fromEntries(Object.entries(DIFFSTATS.bossStrikesHist).sort((a,b)=>a[0]-b[0]).map(([k,v])=>[k,+(v/Math.max(1,DIFFSTATS.bossFights)*100).toFixed(1)])),
      bossStrikesMedian:diffPct(Object.entries(DIFFSTATS.bossStrikesHist).flatMap(([k,v])=>new Array(v).fill(+k)),0.5),
      pctFightsOvertime:+(DIFFSTATS.overtimeFights/Math.max(1,DIFFSTATS.fightsFought)*100).toFixed(1),
      // loss anatomy
      deathStoned:+(DIFFSTATS.deathCause.stoned/Math.max(1,DIFFSTATS.deathCause.stoned+DIFFSTATS.deathCause.beaten)*100).toFixed(1),
      deathsByFight:deathsByFight.map(d=>+(d/G*100).toFixed(2)),
      bandAtkMedByFight:DIFFSTATS.bandAtkByFight.map(a=>Math.round(diffPct(a,0.5))),
      dmgMedByFight:DIFFSTATS.dmgByFight.map(a=>Math.round(diffPct(a,0.5))),
      dmgP25ByFight:DIFFSTATS.dmgByFight.map(a=>Math.round(diffPct(a,0.25))),
      dmgP75ByFight:DIFFSTATS.dmgByFight.map(a=>Math.round(diffPct(a,0.75))),
      strikesMedByFight:DIFFSTATS.strikesByFight.map(a=>diffPct(a,0.5)),
      passRateByFight:DIFFSTATS.fightsFoughtAt.map((n,f)=>+(DIFFSTATS.fightsWonAt[f]/Math.max(1,n)).toFixed(4)),
      fightsFoughtAt:DIFFSTATS.fightsFoughtAt,
    }
  }
  console.log('###LABJSON###'+JSON.stringify(out))
  if(LAB.quiet)process.exit(0)
}
